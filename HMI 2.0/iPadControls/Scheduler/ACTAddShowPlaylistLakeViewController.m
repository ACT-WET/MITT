//
//  ACTAddShowPlaylistLakeViewController.m
//  iPadControls
//
...
@interface ACTAddShowPlaylistLakeViewController ()
...
@property (nonatomic) UIDatePicker *timPicker;
...
@implementation ACTAddShowPlaylistLakeViewController
...
-(void)getCurrentScheduleAndButtonTag{
    ...
    _allPlaylists = [[NSMutableArray alloc] initWithArray:[_defaults objectForKey:@"lakeplaylists"]];
    _shows = [[NSMutableArray alloc] initWithArray:[_defaults objectForKey:@"lakeshows"]];
    
    //Get button tag from user defaults
    _buttonTag = [[_defaults objectForKey:@"lakebuttonTag"] intValue];
...
-(void)createShowTableView{
    ...
    _timPicker = [[UIDatePicker alloc] initWithFrame:CGRectMake(0, 365, 400, 250)];
    _timPicker.datePickerMode = UIDatePickerModeTime;
    _timPicker.hidden = NO;
    
    NSDate *today= _timPicker.date;
    ...
    _timPicker.date =  [gregorian dateFromComponents:components];
    [_timPicker addTarget:self action:@selector(changeDateInLabel:) forControlEvents:UIControlEventValueChanged];
    _selectedTime =  [[NSString stringWithFormat:@"%@", [_timeFormatter stringFromDate:_timPicker.date]] intValue];
    
    [self.view addSubview:_timPicker];
...
-(void)changeDateInLabel:(id)sender{
    ...
    _selectedTimeMin =  [[NSString stringWithFormat:@"%@", [_timeFormatter stringFromDate:_timPicker.date]] intValue];
    _invalidTime.alpha = 0;
...
-(void)saveSchedule{
    ...
    [_defaults setObject:[NSNumber numberWithInt: (int)1] forKey:@"lakedismiss"];
