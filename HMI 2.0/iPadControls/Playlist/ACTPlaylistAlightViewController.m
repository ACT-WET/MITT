//
//  ACTPlaylistAlightViewController.m
//  iPadControls
//
...
#import "ACTSelectShowAlightViewController.h"
...
@interface ACTPlaylistAlightViewController () <UITextFieldDelegate> {
...
@implementation ACTPlaylistAlightViewController{
    ACTSelectShowAlightViewController *controller;
...
-(void)readJSONFile{
    ...
    [manager GET:fullpath parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject){
        ...
        [manager GET:fullpath parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject){
            ...
            NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
            [defaults setObject:responseObject forKey:@"alshows"];
...
-(void)checkAutoHandMode{
    ...
    if (_state == 0){
        ...
        int playMode = [[defaults objectForKey:@"alplayStatus"] intValue];
        ...
        int backWashRunningStat = [[defaults objectForKey:@"lakebackWashRunningStat"] intValue];
...
-(void)openPopupShowSelector{
    ...
    controller = [[ACTSelectShowAlightViewController alloc] init];
...
-(void)dismissPopoverView{
    ...
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    int dismiss = [[defaults objectForKey:@"aldismiss"] intValue];
    ...
    if (dismiss){
        ...
        [defaults setObject:0 forKey:@"aldismiss"];

        selectedShow = [[defaults objectForKey:@"alselectedShow"] intValue];
        _loopCount = [[defaults objectForKey:@"alloopCount"] intValue];
...
-(IBAction)toggleAutoHandMode:(UIButton *)sender{
    ...
    if (_state == 0){
        ...
        newState = 1;
        [defaults setObject:[NSNumber numberWithInt:(int)1] forKey:@"altoggledStatus"];
        
    }else{
        
        newState = 0;
        [defaults setObject:[NSNumber numberWithInt:(int)0] forKey:@"altoggledStatus"];
    ...
    if (cellIndex < _rowCount){
        ...
        if ([newShowName length]){
            ...
            if (![newShowName isEqualToString:_textfieldText]){
                ...
                NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
                [defaults setObject:_shows forKey:@"alshows"];
...
-(void)getCurrentShowInformation{
    ...
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    NSString *showScannerOn = [defaults objectForKey:@"alscanningShows"];
    
    if ([showScannerOn isEqualToString:@"0"]){
        
        int currentShowNumber = [[defaults objectForKey:@"alcurrentShowNumber"] intValue];
