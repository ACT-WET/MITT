//
//  AlightShowStopperChecker.swift
//  iPadControls
//
...
class AlightShowStopperChecker: UIViewController {
   ...
   @IBOutlet var showStopperView: UIView!
        ...
        override func viewWillAppear(_ animated: Bool) {
            timer = Timer.scheduledTimer(timeInterval: 1, target: self, selector: #selector(getShowStoppers), userInfo: nil, repeats: true)
        ...
        @objc private func getShowStoppers(){
            ...
            CENTRAL_SYSTEM?.readBits(plcIpAddress: MITT_LA_PLC_IP_ADDRESS, length: Int32(SHOW_STOPPERS_PLC_REGISTERS.count), startingRegister: Int32(SHOW_STOPPERS_PLC_REGISTERS.startAddress), completion: { (success, response) in
        ...
        private func processShowStoppers(response:[AnyObject]){
                   ...
                   //SS601 Show Stopper
                   let fire_alarm_showstopper        = Int(truncating: response[1] as! NSNumber)
...
//                   }
                   ...
                   if fire_alarm_showstopper == FAULT_DETECTED{
