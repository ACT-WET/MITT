            ...
            if sender.state == .ended {
                if iPadNumber == 1{
                    if convertedFrequency < 10{
                        CENTRAL_SYSTEM?.writeRegister(plcIpAddress: MITT_LAG_PLC_IP_ADDRESS, register: 2, value: 0)
                        DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
                            self.readManualSpeedPLC = true
                        }
                    }else{
                        CENTRAL_SYSTEM?.writeRegister(plcIpAddress: MITT_LAG_PLC_IP_ADDRESS, register: 2, value: convertedFrequency)
                        DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
                            self.readManualSpeedPLC = true
                        }
                    }
                    ...
                    if convertedFrequency < 10{
                        CENTRAL_SYSTEM?.writeRegister(plcIpAddress: MITT_LAG_PLC_IP_ADDRESS, register: 22, value: 0)
                        DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
                            self.readManualSpeedPLC = true
                        }
                    }else{
                        CENTRAL_SYSTEM?.writeRegister(plcIpAddress: MITT_LAG_PLC_IP_ADDRESS, register: 22, value: convertedFrequency)
                        DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
                            self.readManualSpeedPLC = true
                        }
                    }
            ...
            if sender.state == .ended {
                if iPadNumber == 1{
                    if convertedBWFrequency < 10{
                        CENTRAL_SYSTEM?.writeRegister(plcIpAddress: MITT_LAG_PLC_IP_ADDRESS, register: 11, value: 0)
                    }else{
                        CENTRAL_SYSTEM?.writeRegister(plcIpAddress: MITT_LAG_PLC_IP_ADDRESS, register: 11, value: convertedBWFrequency)
                    }
                    DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
                    ...
                    if convertedBWFrequency < 10{
                        CENTRAL_SYSTEM?.writeRegister(plcIpAddress: MITT_LAG_PLC_IP_ADDRESS, register: 31, value: 0)
                    }else{
                        CENTRAL_SYSTEM?.writeRegister(plcIpAddress: MITT_LAG_PLC_IP_ADDRESS, register: 31, value: convertedBWFrequency)
                    }
                    DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
                        self.readBackWashSpeedOnce = false
                    }
