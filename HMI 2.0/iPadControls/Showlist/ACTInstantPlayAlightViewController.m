//
//  ACTInstantPlayAlightViewController.m
//  iPadControls
//
...
#import "ACTInstantPlayAlightViewController.h"
#import "AFHTTPRequestOperationManager.h"
#import "iPadControls-Swift.h"

@interface ACTInstantPlayAlightViewController ()
...
@property (nonatomic) NSMutableArray *alshows;
...
@implementation ACTInstantPlayAlightViewController
...
-(void)viewWillDisappear:(BOOL)animated{
    ...
    [defaults setObject:[NSNumber numberWithInt:0] forKey:@"alinstantPlaySelectedShow"];
...
-(void)initializeFile{
    ...
    _state = [[defaults objectForKey:@"alplayMode"] intValue];
    [defaults setObject:[NSNumber numberWithInt:_state] forKey:@"altoggledStatus"];
...
-(void)readInternalShowFile{
    ...
    if ([defaults objectForKey:@"alshows"]){
        
        _alshows = [[NSMutableArray alloc] initWithArray:[defaults objectForKey:@"alshows"]];
...
-(void)checkForConnection{
    ...
    NSString *plcConnectionStatus   = [defaults objectForKey:@"ATALPLCConnectionStatus"];
    
    if ([plcConnectionStatus isEqualToString:@"ATALPLCConnected"] && [serverConnectionStatus isEqualToString:@"serverConnected"]){
...
-(void)getCurrentShowInfo{
    ...
    int showPlaying = [[defaults objectForKey:@"alplayStatus"] intValue];
    int currentShow = [[defaults objectForKey:@"alcurrentShowNumber"] intValue];
    int playType = [[defaults objectForKey:@"alShow Type"] intValue];
    
    if (playType == 1){
        currentShow = currentShow - 1024;
        NSLog(@"Current Show:  %d",currentShow);
    }
    NSString *showName = [defaults objectForKey:@"alcurrentShowName"];
    NSLog(@"Current Show:  %@",showName);
    NSString *dateStr =  [defaults objectForKey:@"aldeflate"];
    ...
    if (showPlaying && currentShow != 0){
        ...
        int showremaining = [[defaults objectForKey:@"alshow time remaining"] intValue];
        int showDuration = [[defaults objectForKey:@"alshowDuration"] intValue] - showremaining;
...
-(void)readStopButton{
    ...
    int toggledValue = [[defaults objectForKey:@"altoggledStatus"] intValue];
    ...
    int showPlayingStatus = [[defaults objectForKey:@"alplayStatus"] intValue];
    int currentShow = [[defaults objectForKey:@"alcurrentShowNumber"] intValue];
    int playType = [[defaults objectForKey:@"alShow Type"] intValue];
...
-(IBAction)toggleAutoHandMode:(UIButton *)sender{
    ...
    if (_state == 0){
        ...
        newState = 1;
        [_defaults setObject:[NSNumber numberWithInt:(int)1] forKey:@"altoggledStatus"];

    }else{
    
        newState = 0;
        [_defaults setObject:[NSNumber numberWithInt:(int)0] forKey:@"altoggledStatus"];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    int duration = [[[_alshows objectAtIndex:indexPath.row + 1] objectForKey:@"duration"] intValue];
    int showNumber = [[[_alshows objectAtIndex:indexPath.row + 1] objectForKey:@"number"] intValue];
    int min = duration/60;
    int sec = duration % 60;
    ...
    _selectedShow = [[_defaults objectForKey:@"alinstantPlaySelectedShow"] intValue];
    
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
        cell.textLabel.text = [[_alshows objectAtIndex:indexPath.row + 1] objectForKey:@"name"];
...
-(void)playShow:(NSIndexPath *)indexPath{
    ...
    if(![cell.detailTextLabel.text isEqual:@"00:00"]){
        ...
        _selectedShow = [[[_alshows objectAtIndex:indexPath.row + 1] objectForKey:@"number"] intValue];
        _selectedShowDuration = [[[_alshows objectAtIndex:indexPath.row + 1] objectForKey:@"duration"] intValue];
        
        //If Show System is in Manual Mode then highlight the show on selection
        if (_state == 1){
            
            [defaults setObject:[NSNumber numberWithInt:(int)indexPath.row + 1] forKey:@"alinstantPlaySelectedShow"];
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
            cell.detailTextLabel.textColor = [UIColor colorWithRed:130.0f/255.0f green:180.0f/255.0f blue:0.0f/255.0f alpha:1.0f];
            cell.textLabel.textColor = [UIColor colorWithRed:130.0f/255.0f green:180.0f/255.0f blue:0.0f/255.0f alpha:1.0f];
        }
    ...
    if ([[[_alshows objectAtIndex:indexPath.row + 1] objectForKey:@"test"] intValue] == 1){
