//
//  LakeSettingsViewController.swift
//  iPadControls
//
...
class LakeSettingsViewController: UIViewController {
    ...
    @IBAction func syncServerTimer(_ sender: Any){
      syncTimeToServer()
        ...
        self.httpManager.httpGetResponseFromPath(url:RESET_TIME_LAST2_COMMAND){ (response) in
    ...
    func syncTimeToServer(){
     ...
     self.syncTimeStateLbl.text = "SYNCING SERVER TIME..."
           ...
           DispatchQueue.global(qos: .background).async{
               ...
               self.session = NMSSHSession.connect(toHost: "\(SERVER_IP2_ADDRESS):22", withUsername: "root")
    ...
    @objc func getDateTime(){
        ...
        ipadDateLbl.text = SERVER2_TIME
        
    }
    
    @IBAction func serverReset(_ sender: UIButton) {
        self.rebootMsg.text = "REBOOTING SERVER IN 15 SEC"
        CENTRAL_SYSTEM?.writeBit(plcIpAddress: MITT_LA_PLC_IP_ADDRESS, bit: SERVER_REBOOT_BIT, value: 1)
        self.resetBtn.isUserInteractionEnabled = false
        DispatchQueue.main.asyncAfter(deadline: .now() + 15.0) {
            self.rebootMsg.text = "REBOOT SUCCESS"
            CENTRAL_SYSTEM?.writeBit(plcIpAddress: MITT_LA_PLC_IP_ADDRESS, bit: SERVER_REBOOT_BIT, value: 0)
