//
//  LakeWindViewController.swift
//  iPadControls
//
//  Created by Rakesh Raveendra on 6/8/23.
//  Copyright © 2023 WET. All rights reserved.
//

import UIKit

class LakeWindViewController: UIViewController {
    ...
    private func readWindSensorData(){
            ...
            CENTRAL_SYSTEM?.readRegister(plcIpAddress: MITT_LA_PLC_IP_ADDRESS, length: 1, startingRegister: Int32(WIND_SPM_HAND_MODE), completion:{ (success, response) in
    ...
    private func getChannelFaults(){
        ...
        CENTRAL_SYSTEM?.readBits(plcIpAddress: MITT_LA_PLC_IP_ADDRESS, length: 1, startingRegister: Int32(WIND_MODE), completion:{ (success, response) in
    ...
    @IBAction func showSetpoints(_ sender: UIButton){
            ...
            let popoverContent = storyboard.instantiateViewController(withIdentifier: "windLakeDetails") as! LakeWindAnemometerViewController
    ...
    @IBAction func frceSPMMode(_ sender: UIButton) {
        if sender.tag == 891{
            CENTRAL_SYSTEM!.writeRegister(plcIpAddress: MITT_LA_PLC_IP_ADDRESS, register: WIND_SPM_HAND_MODE, value: 0)
        }
        if sender.tag == 892{
            CENTRAL_SYSTEM!.writeRegister(plcIpAddress: MITT_LA_PLC_IP_ADDRESS, register: WIND_SPM_HAND_MODE, value: 1)
        }
        if sender.tag == 893{
            CENTRAL_SYSTEM!.writeRegister(plcIpAddress: MITT_LA_PLC_IP_ADDRESS, register: WIND_SPM_HAND_MODE, value: 2)
        }
        if sender.tag == 894{
            CENTRAL_SYSTEM!.writeRegister(plcIpAddress: MITT_LA_PLC_IP_ADDRESS, register: WIND_SPM_HAND_MODE, value: 3)
        }
    }
    ...
    @IBAction func changeMode(_ sender: UIButton) {
        if sender.tag == 1{
            CENTRAL_SYSTEM?.writeBit(plcIpAddress: MITT_LA_PLC_IP_ADDRESS, bit: WIND_MODE, value: 0)
            CENTRAL_SYSTEM!.writeRegister(plcIpAddress: MITT_LA_PLC_IP_ADDRESS, register: WIND_SPM_HAND_MODE, value: 5)
        }
        if sender.tag == 2{
            CENTRAL_SYSTEM?.writeBit(plcIpAddress: MITT_LA_PLC_IP_ADDRESS, bit: WIND_MODE, value: 1)
        }
    }
}
