...
class WindAnemometerViewController: UIViewController,UIGestureRecognizerDelegate{
    ...
    @objc func changeAboveSP(sender: UIPanGestureRecognizer){
        ...
        if touchLocation.y > 35.7 && touchLocation.y <= mediumylocation - 8.62 {
            ...
            if sender.state == .ended {
                ...
                CENTRAL_SYSTEM?.writeRealValue(plcIpAddress: MITT_LAG_PLC_IP_ADDRESS, register: self.highSP, value: Float(formattedHerts)!)
    ...
    @objc func changeMediumSP(sender: UIPanGestureRecognizer){
        ...
        if touchLocation.y > aboveylocation + 42 && touchLocation.y <= belowylocation {
            ...
            if sender.state == .ended {
                ...
                CENTRAL_SYSTEM?.writeRealValue(plcIpAddress: MITT_LAG_PLC_IP_ADDRESS, register: self.medSP, value: Float(formattedHerts)!)
                DispatchQueue.main.asyncAfter(deadline: .now() + 1.50) {
                    self.readSetpointAgain()
                }
            }
    ...
    @objc func changeBelowSP(sender: UIPanGestureRecognizer){
        ...
        if touchLocation.y >= mediumylocation + 25.86 && touchLocation.y < 468 {
            ...
            if sender.state == .ended {
                ...
                CENTRAL_SYSTEM?.writeRealValue(plcIpAddress: MITT_LAG_PLC_IP_ADDRESS, register: self.lowSP, value: Float(formattedHerts)!)
                DispatchQueue.main.asyncAfter(deadline: .now() + 1.50) {
                    self.readSetpointAgain()
                }
            }
    ...
    private func getWindSpeedReading(windSpeedRegister:Int, windAbortSetpoint:Int, aboveHigh:Int, aboveMed:Int, belowLow:Int){
        ...
        if self.fetchedSetpoints != 1{
            ...
            CENTRAL_SYSTEM!.readRealRegister(plcIpAddress: MITT_LAG_PLC_IP_ADDRESS, register: windSpeedRegister, length: 2){ (success, response)  in
    ...
    private func readSetpoints(aboveHighSetPoint: Int, mediumSetpoint:Int, belowLowSetpoint: Int, abortSP: Int){
        CENTRAL_SYSTEM!.readRealRegister(plcIpAddress: MITT_LAG_PLC_IP_ADDRESS, register: abortSP, length: 2){ (success, response)  in
            ...
            CENTRAL_SYSTEM!.readRealRegister(plcIpAddress: MITT_LAG_PLC_IP_ADDRESS, register: aboveHighSetPoint, length: 2){ (success, response)  in
                ...
                CENTRAL_SYSTEM!.readRealRegister(plcIpAddress: MITT_LAG_PLC_IP_ADDRESS, register: mediumSetpoint, length: 2){ (success, response)  in
                    ...
                    CENTRAL_SYSTEM!.readRealRegister(plcIpAddress: MITT_LAG_PLC_IP_ADDRESS, register: belowLowSetpoint, length: 2){ (success, response)  in
...
//            print("below lo set point successful. above high sp is \(below_l)")
    ...
    @IBAction func setNewAbortSPButtonPressed(_ sender: Any) {
        if let setpoint = abortSPTextField.text, !setpoint.isEmpty,
            ...
            CENTRAL_SYSTEM?.writeRealValue(plcIpAddress: MITT_LAG_PLC_IP_ADDRESS, register: currentAbortSetpointRegister, value: floatValue)
            readSetpointAgain()
        } else {
            print("EMPTY FIELD")
        }
