...
class FillerShowsViewController: UIViewController,UITextViewDelegate, UIPickerViewDelegate, UIPickerViewDataSource, UITextFieldDelegate{
    ...
    func readFillerShowOnOff(){
        CENTRAL_SYSTEM?.readBits(plcIpAddress: MITT_LAG_PLC_IP_ADDRESS, length: 1, startingRegister: Int32(FILLER_SHOW_STATUS_WRITE_REGISTER), completion: { (success, response) in
