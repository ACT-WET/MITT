...
class SystemStatusViewController: UIViewController {
    ...
    private func acquireDataFromPLC(){
        ...
        CENTRAL_SYSTEM?.readRegister(plcIpAddress: MITT_LAG_PLC_IP_ADDRESS, length: 1, startingRegister: Int32(SYSTEM_FAULT_YELLOW), completion:{ (success, response) in
...
//                self.yellowStateResp = 15
                ...
                yPosition += offset
                    if faultTag == 1{
                        ...
                        for index in 0...STRAINER_STATUS.count - 1 {
                            CENTRAL_SYSTEM?.readBits(plcIpAddress: MITT_LAG_PLC_IP_ADDRESS, length: 1, startingRegister: Int32(STRAINER_STATUS.startingregister+index), completion:{ (success, response) in
                ...
                if state == 0 {
                    ...
                    if faultTag == 14{
                        ...
                        for index in 0...ETHERNET_STATUS.count - 1 {
                            CENTRAL_SYSTEM?.readBits(plcIpAddress: MITT_LAG_PLC_IP_ADDRESS, length: 1, startingRegister: Int32(ETHERNET_STATUS.startingregister+index), completion:{ (success, response) in
    ...
    @IBAction func faultResetBtnPushed(_ sender: Any) {
        ...
        CENTRAL_SYSTEM?.writeBit(plcIpAddress: MITT_LAG_PLC_IP_ADDRESS, bit: FAULT_RESET_REGISTER, value: 1)
        self.faultBtn.isUserInteractionEnabled = false
        self.faultBtn.isEnabled = false
        DispatchQueue.main.asyncAfter(deadline: .now() + .seconds(1), execute:{
            CENTRAL_SYSTEM?.writeBit(plcIpAddress: MITT_LAG_PLC_IP_ADDRESS, bit: FAULT_RESET_REGISTER, value: 0)
    ...
    @IBAction func warningResetBtnPushed(_ sender: Any) {
        CENTRAL_SYSTEM?.writeBit(plcIpAddress: MITT_LAG_PLC_IP_ADDRESS, bit: WARNING_RESET_REGISTER, value: 1)
        self.warningbtn.isUserInteractionEnabled = false
        self.warningbtn.isEnabled = false
        DispatchQueue.main.asyncAfter(deadline: .now() + .seconds(1), execute:{
            CENTRAL_SYSTEM?.writeBit(plcIpAddress: MITT_LAG_PLC_IP_ADDRESS, bit: WARNING_RESET_REGISTER, value: 0)
