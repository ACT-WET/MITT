...
class WeekSchedulerViewController: UIViewController, UIPopoverPresentationControllerDelegate {
    ...
    @objc func readLightsORPumpsData() {
        ...
        if !readScheduleOnce {
            ...
            self.httpComm.httpGetResponseFromPath(url: "\(HTTP_PASS)\(IPADDR):8080/\(readServerPath)"){ (response) in
    ...
    private func writeSchedule(){
        ...
        httpComm.httpGet(url: "\(HTTP_PASS)\(IPADDR):8080/\(writeServerPath)?\(escapedDataString!)") { (response, success) in
            if success == true {
                readScheduleOnce = false
            }
        }
