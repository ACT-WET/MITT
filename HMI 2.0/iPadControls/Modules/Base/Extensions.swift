...
extension UIViewController {
    ...
    @objc public func checkScanStatus(){
           ...
           httpComm.httpGetResponseFromPath(url: READ_SHOWSCAN_STATUS){ (response) in
               ...
               if (state == 1){
                   ...
                   UserDefaults.standard.set(totalNumOfTestShows, forKey: "numberOfSPMTestShows")
                   UserDefaults.standard.set(currentTestShow, forKey: "testshowScanned")
               }
           }
       }
    ...
    @objc public func checkLakeCurrentSchedule(){
        let httpComm = HTTPComm()
        let calendar = Calendar.current
        let currentDate = Date()
        let components = calendar.dateComponents([.month, .day], from: currentDate)
        ...
        httpComm.httpGetResponseFromPath(url: READ_LAKE_TIME_TABLE){ (response) in
